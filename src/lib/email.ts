...
class EmailService {
    ...
    private getEmailConfig(): EmailConfig {
        ...
        return {
            provider,
            from: process.env.EMAIL_FROM || "noreply@scrybe.chat",
            resend:
                provider === "resend"
                    ? {
    ...
    async sendVerificationEmail(data: {
        ...
        await this.sendEmail({
            to: data.user.email,
            subject: "Verify your email address - scrybe Chat",
            html,
    ...
    async sendPasswordResetEmail(data: {
        ...
        await this.sendEmail({
            to: data.user.email,
            subject: "Reset your password - scrybe Chat",
            html,
    ...
    async sendOTPEmail(data: {
        ...
        const getSubjectAndTemplate = async () => {
            switch (data.type) {
                case "sign-in":
                    return {
                        subject: "Your sign-in code - scrybe Chat",
                        html: await render(
                            OTPEmailTemplate({
                                otp: data.otp,
                                type: "sign-in"
                        ...
                        text: `Your sign-in code for scrybe Chat is: ${data.otp}\n\nThis code will expire in 5 minutes.`
                    }
                case "email-verification":
                    return {
                        subject: "Verify your email - scrybe Chat",
                        html: await render(
                            OTPEmailTemplate({
                                otp: data.otp,
                        ...
                        text: `Your email verification code for scrybe Chat is: ${data.otp}\n\nThis code will expire in 5 minutes.`
                    }
                case "forget-password":
                    return {
                        ...
                        text: `Your password reset code for scrybe Chat is: ${data.otp}\n\nThis code will expire in 5 minutes.`
                    }
            }
        }
